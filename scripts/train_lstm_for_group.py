# this is the first version of lstm, use dl 1536 features as input
...
out_dir = '//lyta/tomodev/DeepLearning/Playground_stage3/output_groupProc/lstm_based_model/score_txt/v1/'
...
def run():
    ...
    train_x = lstm_processing.form_lstm_features(train_detections)
    ...
    test_x = lstm_processing.form_lstm_features(test_detections)
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(1024, return_sequences=True), input_shape=(maxlen, 1536)))
    ...
    for fn in glob.glob('C:/experiments/temp_feature_vector/test/*.txt'):
        ...
        test_x = lstm_processing.form_lstm_features(test_detections_one_view)
        test_x = sequence.pad_sequences(test_x, maxlen=maxlen, dtype='float')
        test_x = np.array(test_x)

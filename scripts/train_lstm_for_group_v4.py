# this is the 4th version, 3 feature for lstm input. insert 0s for non detection slices.
...
out_dir = '//lyta/tomodev/DeepLearning/Playground_stage3/output_groupProc/lstm_based_model/score_txt/v4/'
...
def run():
    ...
    train_x = lstm_processing.insert_zero_form_sequence(train_x) # must use form_lstm_features_v3 to use this function!!!
    ...
    test_x = lstm_processing.insert_zero_form_sequence(test_x)
    ...
    for fn in glob.glob('C:/experiments/temp_feature_vector/test/*.txt'):
        ...
        test_x = lstm_processing.form_lstm_features_v3(test_detections_one_view)
        test_x = lstm_processing.insert_zero_form_sequence(test_x)
        test_x = sequence.pad_sequences(test_x, maxlen=maxlen, dtype='float')
        test_x = np.array(test_x)
